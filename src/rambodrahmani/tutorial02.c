 ...
 *
 *           Compiled using
 *               gcc -o tutorial02 tutorial02.c -lavutil -lavformat -lavcodec -lswscale -lz -lm
 *`sdl2-config --cflags --libs` on Arch Linux. sdl-config just prints out the proper flags for gcc
 *to include the SDL libraries properly. You may need to do something different to get it to compile
 *on your system; please check the SDL documentation for your system. Once it compiles, go ahead and
 *run it.
 *
...
int main_tutorial02(int argc, char* argv[]) {
  ...
  SDL_Window* screen = SDL_CreateWindow(  // [2]
      "SDL Video Player", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, pCodecCtx->width / 2,
      pCodecCtx->height / 2, SDL_WINDOW_OPENGL | SDL_WINDOW_ALLOW_HIGHDPI);
  ...
  renderer = SDL_CreateRenderer(
      screen, -1,
      SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC | SDL_RENDERER_TARGETTEXTURE);  // [3]
  ...
  texture = SDL_CreateTexture(  // [4]
      renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING, pCodecCtx->width,
      pCodecCtx->height);
  ...
  av_image_fill_arrays(pict->data, pict->linesize, buffer, AV_PIX_FMT_YUV420P, pCodecCtx->width,
                       pCodecCtx->height, 32);
      ...
      while (ret >= 0) {
        ...
        if (++i <= maxFramesToDecode) {
          ...
          printf(
              ...
              av_get_picture_type_char(pFrame->pict_type), pCodecCtx->frame_number, pFrame->pts,
              pFrame->pkt_dts, pFrame->key_frame, pFrame->coded_picture_number,
              pFrame->display_picture_number, pCodecCtx->width, pCodecCtx->height);

          // Use this function to update a rectangle within a planar
          // YV12 or IYUV texture with new pixel data.
          SDL_UpdateYUVTexture(
              texture,  // the texture to update
              &rect,    // a pointer to the rectangle of pixels to update, or NULL to update the
                        // entire texture
